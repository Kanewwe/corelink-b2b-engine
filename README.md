...
corelink-b2b-engine/
├── backend/
│   ├── main.py              # FastAPI 主程式 (Auth 與核心路由)
│   ├── models.py            # SQLAlchemy 模型 (Lead, GlobalLead, SystemSettings)
│   ├── database.py          # 資料庫引擎 (PostgreSQL + SSL + Schema 切換)
│   ├── manufacturer_miner.py # 製造商探勘核心 (v3.1.8 現代化驅動 + 超時保護)
│   ├── scrape_utils.py       # 探勘輔助工具 (全域同步與去重邏輯)
│   ├── email_hunter.py      # Email 發現與猜測邏輯
│   ├── classifier.py        # AI 產業分類服務
│   └── requirements.txt
├── frontend/                # React (Vite) 前端
├── docs/                    # 完整技術開發手冊
│   ├── DATABASE.md          # 資料庫詳細 Schema 與索引說明
│   ├── SCRAPER_DOCUMENTATION.md # 探勘引擎 4 階段運作原理
│   ├── LEAD_ISOLATION_ARCHITECTURE.md # 全域庫與私域庫同步架構 (New)
│   ├── DATABASE_ENV.md      # 多環境 (UAT/PRD) 管理指南
│   └── RENDER_SETUP_GUIDE.md # 雲端部署與 SSL 配置經驗
├── start.sh                 # 容器啟動腳本
├── render.yaml              # 基礎設施即代碼 (IaC) 定義
└── README.md
```
