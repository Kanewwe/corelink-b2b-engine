...
### 最新更新 (2026-03-26) - v2.7.1

**全域隔離池與智慧探勘控制**：
- 🌐 **全域探勘隔離池 (Global Lead Pool)**：領先業界的「資料庫優先」探勘機制，自動同步跨使用者的高質量 Lead，節省外部 API 點數與採集時間。
- 📊 **系統設定 General 面板**：實時監控隔離池容量、Domain 佔比與產業分佈。
- ⚡ **智慧探勘開關**：管理員可根據預算隨時切換全域同步或即時爬取模式。
- 🏷️ **視覺化來源標籤**：Lead 列表標註 `GLOBAL SYNC` 或 `LIVE SCRAPE`，透明化資料來源。
